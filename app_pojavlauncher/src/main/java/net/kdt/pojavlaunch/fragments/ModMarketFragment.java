...
public class ModMarketFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mAdapter.setPickerListener(new MarketModAdapter.OnVersionPickerListener() {
            ...
            public void onVersionPicker(JSONObject mod, TextView picker) {
                showVersionPickerForCard(picker);
            }
            @Override
            public void onLoaderPicker(JSONObject mod, TextView picker) {
                showLoaderPickerForCard(picker);
            }
        });
    ...
    private void showVersionPickerForCard(TextView picker) {
        String[] versions = mGameVersions != null && mGameVersions.length > 0
        ...
        new androidx.appcompat.app.AlertDialog.Builder(requireContext())
            ...
            .setItems(versions, (d, which) -> {
                if (which == 0) {
                    picker.setText("All Game Versions ▾");
                    picker.setTag(null);
                } else {
                    picker.setText(versions[which] + " ▾");
                    picker.setTag(versions[which]);
                }
            })
            .show();
    }

    private void showLoaderPickerForCard(TextView picker) {
        String[] loaders = {"All loaders", "Fabric", "Forge", "NeoForge", "Quilt"};
        new androidx.appcompat.app.AlertDialog.Builder(requireContext())
            .setTitle("Mod loader")
            .setItems(loaders, (d, which) -> {
                if (which == 0) {
                    picker.setText("All Mod Loaders ▾");
                    picker.setTag(null);
                } else {
                    picker.setText(loaders[which] + " ▾");
                    picker.setTag(loaders[which].toLowerCase());
                }
            })
            .show();
    }
