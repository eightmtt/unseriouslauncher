...
public class ModMarketFragment extends Fragment {
    ...
    private int mPage = 1; // текущая страница (1-based)
        ...
        mExecutor.execute(() -> {
            try {
                ...
                if (source == SOURCE_MODRINTH) {
                    ...
                    results = ModrinthApi.searchMods(query, version, rawVersion, offset, PAGE_SIZE, totalHolder);
                    total = totalHolder[0];
                } else {
                    int[] totalHolder = new int[1];
                ...
                mHandler.post(() -> {
                    hideLoading();
                    mTotalPages = (int) Math.ceil((double) t / PAGE_SIZE);
    ...
    private void onModClick(JSONObject mod) {
        if (mSource == SOURCE_MODRINTH) {
            showModrinthVersions(mod);
        } else {
            showCurseForgeVersions(mod);
        }
    }
            ...
            mExecutor.execute(() -> {
                try {
                    ...
                    mHandler.post(() -> {
                        ...
                        String[] names = buildCurseForgeFileNames(files);
                        new AlertDialog.Builder(requireContext())
                            .setTitle(name)
                            .setItems(names, (d, i) -> {
                                try {
                                    confirmCurseForgeInstall(name, files.getJSONObject(i), version, rawVersion);
            ...
            mExecutor.execute(() -> {
                try {
                    ...
                    mHandler.post(() -> {
                        ...
                        String[] names = buildModrinthVersionNames(versions);
                        new AlertDialog.Builder(requireContext())
                            .setTitle(name)
                            .setItems(names, (d, i) -> {
                                try {
                                    confirmModrinthInstall(name, versions.getJSONObject(i));
                                } catch (Exception ignored) {}
                            })
                            .setNegativeButton("Отмена", null)
                            .show();
