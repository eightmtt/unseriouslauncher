    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        try {
            ...
            if (isExpanded) {
                ...
                if (showSelectors) {
                    // Восстанавливаем текст если был выбор
                    if (holder.mVersionPicker.getTag() == null)
                        holder.mVersionPicker.setText("All Game Versions ▾");
                    if (holder.mLoaderPicker.getTag() == null)
                ...
                holder.mDownloadBtn.setOnClickListener(v -> {
                    String ver = (mFilterEnabled && !mVersionFilter.isEmpty())
                        ? mVersionFilter
                        : (holder.mVersionPicker.getTag() instanceof String ? (String) holder.mVersionPicker.getTag() : null);
                    String loader = (mFilterEnabled && !mLoaderFilter.isEmpty())
                        ? mLoaderFilter
                        : (holder.mLoaderPicker.getTag() instanceof String ? (String) holder.mLoaderPicker.getTag() : null);
        ...
        try {
            ...
            for (int i = 0; i < max; i++) {
                ...
                if (cat instanceof JSONObject) {
                    // CurseForge — категория это объект с полем "name"
                    tag = ((JSONObject) cat).optString("name", "");
                } else {
                    // Modrinth — категория это строка
                    tag = cat.toString();
                }
                ...
                tv.setBackground(container.getContext().getDrawable(R.drawable.background_line));
                tv.setPadding(14, 3, 14, 3);
